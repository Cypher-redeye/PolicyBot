...
class MySQLLogger:
    ...
    def _setup_database(self):
        try:
            ...
            cursor.close()
            print("[OK] MySQL database and tables initialized")
            
        except Exception as e:
            print(f"Warning: MySQL setup error ({e}). Falling back to local SQLite database.")
            self.connection = None
    ...
    def _setup_sqlite(self):
        try:
            db_path = os.path.join(os.path.dirname(os.path.dirname(__file__)), "rag_system.db")
            ...
            cursor.close()
            print(f"[OK] Local SQLite database initialized at {db_path}")
        except Exception as e:
    ...
    def log_query(self, query: str, response: str, context: List[str], 
        ...
        if self.use_sqlite:
            ...
            try:
                ...
                if session_id:
                    cursor.execute("""
                        INSERT OR IGNORE INTO conversation_sessions (session_id, total_queries)
                        VALUES (?, 0)
                    """, (session_id,))
                    cursor.execute("""
                ...
                print(f"Error logging query in SQLite: {e}")
            return
    ...
    def log_document(self, doc_name: str, doc_type: str, num_chunks: int, 
        ...
        if self.use_sqlite:
            ...
            try:
                ...
                print(f"Error logging document in SQLite: {e}")
            return
    ...
    def get_recent_queries(self, limit: int = 10, session_id: Optional[str] = None) -> List[Dict]:
        if self.use_sqlite:
            ...
            try:
                ...
                print(f"Error retrieving queries from SQLite: {e}")
                return []
    ...
    def get_conversation_history(self, session_id: str, limit: int = 20) -> List[Dict]:
        if self.use_sqlite:
            ...
            try:
                ...
                print(f"Error retrieving conversation history from SQLite: {e}")
                return []
